...
def daily_dataframe(forecast_json):
    ...
    if "precipitation_sum" in df.columns:
        df["precipitation_sum"] = pd.to_numeric(df["precipitation_sum"], errors="coerce").apply(safe_rain)
    return df
...
def hourly_dataframe(forecast_json):
    ...
    if "precipitation" in df.columns:
        df["precipitation"] = pd.to_numeric(df["precipitation"], errors="coerce").apply(safe_rain)
    return df
...
def power_dataframe(power_json):
    ...
    df["chuva_mm"] = df["chuva_mm"].apply(safe_rain)
    return df
...
with msg_col:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="title-row"><div><div class="section-title">Mensagens automáticas</div><div class="section-sub">Prévia da manhã e da tarde para copiar</div></div><span class="info-badge">Sem WhatsApp por enquanto</span></div>', unsafe_allow_html=True)
